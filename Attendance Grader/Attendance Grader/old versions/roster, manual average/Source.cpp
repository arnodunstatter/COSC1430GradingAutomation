...
int main()
{
	...
	//else...
	string firstLine;
	getline(read, firstLine); //the key which tells us how many labs there are
	int numLabs = countLabs(firstLine);
	while (!read.eof())
	{
		string temp;
		double grade;
		...
		getline(read, temp, ','); //gets total (trash)
		getline(read, temp, ','); //gets participation total (trash)
		getline(read, temp, ','); //gets challenge total (trash)
		getline(read, temp, ','); //gets lab total (trash)
		...
		for (int i = 0; i < numLabs; ++i)
		{
			...
			Students[name]->labGrades.push_back(stod(temp)); //adds each lab grade to the appropriate student's labGrades vector
			Students[name]->gradeLabs(); //grades the lab and updates the bool passedExercises
		}
	}
	read.close();
